/* 
 * Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.
 */

package com.formance.formance_sdk.models.shared;

import com.fasterxml.jackson.annotation.JsonInclude.Include;
import com.fasterxml.jackson.annotation.JsonInclude;
import com.fasterxml.jackson.annotation.JsonProperty;


public class OrchestrationListWalletsResponseCursor {
    @JsonProperty("data")
    public Wallet[] data;

    public OrchestrationListWalletsResponseCursor withData(Wallet[] data) {
        this.data = data;
        return this;
    }
    
    @JsonInclude(Include.NON_ABSENT)
    @JsonProperty("hasMore")
    public Boolean hasMore;

    public OrchestrationListWalletsResponseCursor withHasMore(Boolean hasMore) {
        this.hasMore = hasMore;
        return this;
    }
    
    @JsonInclude(Include.NON_ABSENT)
    @JsonProperty("next")
    public String next;

    public OrchestrationListWalletsResponseCursor withNext(String next) {
        this.next = next;
        return this;
    }
    
    @JsonProperty("pageSize")
    public Long pageSize;

    public OrchestrationListWalletsResponseCursor withPageSize(Long pageSize) {
        this.pageSize = pageSize;
        return this;
    }
    
    @JsonInclude(Include.NON_ABSENT)
    @JsonProperty("previous")
    public String previous;

    public OrchestrationListWalletsResponseCursor withPrevious(String previous) {
        this.previous = previous;
        return this;
    }
    
    public OrchestrationListWalletsResponseCursor(@JsonProperty("data") Wallet[] data, @JsonProperty("pageSize") Long pageSize) {
        this.data = data;
        this.pageSize = pageSize;
  }
}
